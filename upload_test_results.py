...
def create_and_upload_test_results():
    try:
        ...
        INPUT_FINITE_STATE_ORGANIZATION_CONTEXT=os.environ.get('INPUT_FINITE-STATE-ORGANIZATION-CONTEXT')
    ...
    try:
        ...
        token = finite_state_sdk.get_auth_token(INPUT_FINITE_STATE_CLIENT_ID, INPUT_FINITE_STATE_SECRET)
    ...
    if error==None:
        try:
            response = finite_state_sdk.create_new_asset_version_and_upload_test_results(token, INPUT_FINITE_STATE_ORGANIZATION_CONTEXT, business_unit_id=INPUT_BUSINESS_UNIT_ID, created_by_user_id=INPUT_CREATED_BY_USER_ID, asset_id=INPUT_ASSET_ID, version=INPUT_VERSION, file_path=INPUT_FILE_PATH, product_id=INPUT_PRODUCT_ID, artifact_description=INPUT_ARTIFACT_DESCRIPTION, test_type=INPUT_TEST_TYPE)
            set_multiline_output('response', response)
            asset_version=extract_asset_version(response['launchTestResultProcessing']['key'])
        ...
        if error==None:
            ...
            asset_version_url='https://platform.finitestate.io/artifacts/{asset_id}/versions/{version}'.format(asset_id=INPUT_ASSET_ID, version=asset_version)
